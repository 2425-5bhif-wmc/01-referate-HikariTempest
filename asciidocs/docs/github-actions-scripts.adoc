...
<.> Das Event, das diesen Workflow triggert.
...
<.> Spezifiziert, dass es nur ausgeführt wird, wenn die Aktion (in diesem Fall "git push") auf dem Main-branch passiert.
...
* Weitere Informationen: https://docs.github.com/en/actions/writing-workflows/choosing-when-your-workflow-runs/events-that-trigger-workflows#workflow_run[Pull Request Events - GitHub Actions Docs]
